...
this.onResize();{ 
    ...
    this.plotWidth = (width - this.pad) / this.plotsAcross;
    this.plotHeight = (height - this.pad) / this.plotsDown;
this.dialRadius = min((this.plotWidth - this.pad)  - 50, (this.plotHeight - this.pad) - 50); //Dial Radius sizing arguments
};
    ...
    // Adjust arc size to better fit inside plot - needle circle
    arc(0, 0, this.dialRadius/5, this.dialRadius/5, PI, 5 * PI);
    
    textAlign(CENTER);
    textSize(25);
    text(freqLabel, 0, -(this.plotHeight / 2) + 25);  // Shift text slightly to fit better

    for (var i = 0; i < 9; i++) {
        var x = this.dialRadius * cos(nextTickAngle);
        var x1 = (this.dialRadius - 10) * cos(nextTickAngle);
        var y = this.dialRadius * sin(nextTickAngle);
        var y1 = (this.dialRadius - 10) * sin(nextTickAngle);
